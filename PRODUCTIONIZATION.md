...
> **RECOVERED + RE-HOMED 2026-07-09 (Isaac's directive).** The scalable-publishing CI deleted
> this doc from the standalone during publish (`b17114e`; internal docs are stripped from
> published output by design). Recovered from standalone git `73a5355` and homed HERE — the
> monorepo copy is the publish SOURCE, so this is where internal docs live on git. The baseline
> paragraph below is HISTORICAL (2026-07-05): canonicity has since INVERTED (edit monorepo
> `base/heaven-framework`; the standalone receives via the publisher) and the version is now
> **0.1.35** (`setup.py` drifted at 0.1.29 — fix in the next publish). The plan's build order
> (test floor FIRST) still stands and remains unexecuted: standalone `tests/` has zero .py files.

> Baseline (recon-verified, 2026-07-05 — historical): canonical = THIS repo @ **0.1.30** = origin
> = PyPI = the onionmorph venv; the monorepo vendored copy was synced UP to 0.1.30 today
> (sancrev-alpha `5ae5ae6e`) — no divergence anywhere. Ledger: sanctuary-revolution-alpha
> `fable_state_tables.md` T13.
